...
Day-specific notes:
* Day 10
This day uses z3 to solve the problem. It can be installed with ~nimble install 'z3@#HEAD'~ (it needs a currently untagged fix for nim 2.x).

But it also doesn't work out of the box. You'll need to apply some patches to get it to not segfault:
#+begin_src patch
--- a/z3/z3_api.nim
+++ b/z3/z3_api.nim
@@ -467,7 +467,7 @@
